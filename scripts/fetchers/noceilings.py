...
class NoCeilingsFetcher(Fetcher):
    def planned_urls(self) -> List[str]:
        return [f"{self.config.base_url}/big-boards"]

    def fetch(self):
        manual_path = MANUAL_DIR / "noceilings.json"
        ...
        return {
            ...
            "fetched_at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
            "items": [],
            "notes": "Manual snapshot missing: data/ingest/manual/noceilings.json",
